...
- (CBPeripheral *)retrievePeripheral:(NSUUID*)identifier
{
                  ...
                  RSSI:(NSNumber *)RSSI
{
    ...
    TTLog(@"-----------------------Identifer: %@, name: %@, services: %@, description: %@",[peripheral identifier],[peripheral name],[peripheral services],  [advertisementData description]);
    if (YES/*peripheral.name*/)
    { //VIMOTO T10设备已经去掉了名字，所以在此不检测是否存在名字
        CSRPeripheral *p = [[CSRPeripheral alloc]  initWithCBPeripheral:peripheral advertisementData:advertisementData  rssi:RSSI];
        [self.devices setObject:p forKey:peripheral.identifier];
...
{
    TTLog(@"-------------------Connected to peripheral: %@", peripheral.identifier.UUIDString);
    ...
    if ([self.devices objectForKey:peripheral.name] == nil)
    {
        CSRPeripheral *p = [[CSRPeripheral alloc] initWithCBPeripheral:peripheral  advertisementData:nil  rssi:0];
        [self.devices setObject:p forKey:peripheral.identifier];
    }
...
{
    TTLog(@"------------------------------Disconnecting from peripheral: %@\n%@",  peripheral.identifier.UUIDString, error ? error.localizedDescription : @"");
    [self clearListeners];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error
{
    ...
    {
       // TTLog(@"didDiscoverServices error: %@", error.localizedDescription);
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    if (!error)
    {
        ...
        {
           // TTLog(@"Charcteristic %@ %@",characteristic.UUID,[[NSString alloc] initWithData:[characteristic value] encoding:NSUTF8StringEncoding]);
            [peripheral discoverDescriptorsForCharacteristic:characteristic];
        }
    }
    else
    {
        //TTLog(@"didDiscoverCharacteristicsForService error: %@", error.localizedDescription);
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic  error:(NSError *)error
{
    if (error)
    {
     //   TTLog(@"Charcteristic %@ Error: %@",characteristic.UUID, error.localizedDescription);
    }
}

- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic  error:(NSError *)error {
   // TTLog(@"didUpdateValueForCharacteristic __________________: %@", characteristic);
    ...
    if (!error) {
        ...
        TTLog(@"didUpdateValueForCharacteristic error: %@", error.localizedDescription);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic
    ...
    if (self.characteristicQueue.count > 0) {
        ...
        if (error) {
           // TTLog(@"didWriteValueForCharacteristic error: %@", error.localizedDescription);
...
didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic
             error:(NSError *)error {
    if (!error) {
      //  TTLog(@"didUpdateNotificationStateForCharacteristic %@ %@", characteristic, characteristic.UUID);
       ...
       // TTLog(@"Update peripheral: %@ Error: %@", characteristic.UUID.UUIDString, error.localizedDescription);
    }
}
